...
# نشر الباكيند مع إصلاح مشكلة الرفع
set -e
...
echo -e "${BLUE}"
echo "=========================================="
echo "  🚀 نشر الباكيند مع إصلاح مشكلة الرفع"
echo "  📦 Deploy Backend with Upload Fix"
echo "=========================================="
echo -e "${NC}"
...
# 2. إنشاء Worker بسيط
echo -e "${YELLOW}2. إنشاء Worker بسيط...${NC}"
...
echo -e "${GREEN}✅ تم إنشاء Worker بسيط${NC}"

# 3. رفع Worker باستخدام طريقة مختلفة
echo -e "${YELLOW}3. رفع Worker باستخدام طريقة مختلفة...${NC}"

# محاولة 1: رفع مباشر
echo -e "${YELLOW}محاولة 1: رفع مباشر...${NC}"
UPLOAD_RESPONSE1=$(curl -s -X PUT "https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/workers/scripts/cursor-backend" \
  -H "Authorization: Bearer $API_TOKEN" \
  -F "script=@simple-backend.js")

echo "Upload Response 1: $UPLOAD_RESPONSE1"

if echo "$UPLOAD_RESPONSE1" | grep -q '"success":true'; then
    echo -e "${GREEN}✅ تم رفع الباكيند بنجاح (طريقة 1)${NC}"
else
    echo -e "${YELLOW}⚠️  فشل الطريقة 1، جاري المحاولة بالطريقة 2${NC}"
    
    # محاولة 2: رفع مع Content-Type
    echo -e "${YELLOW}محاولة 2: رفع مع Content-Type...${NC}"
    UPLOAD_RESPONSE2=$(curl -s -X PUT "https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/workers/scripts/cursor-backend" \
      -H "Authorization: Bearer $API_TOKEN" \
    ...
    if echo "$UPLOAD_RESPONSE2" | grep -q '"success":true'; then
        echo -e "${GREEN}✅ تم رفع الباكيند بنجاح (طريقة 2)${NC}"
    else
        echo -e "${YELLOW}⚠️  فشل الطريقة 2، جاري المحاولة بالطريقة 3${NC}"
        
        # محاولة 3: رفع مع wrangler
        echo -e "${YELLOW}محاولة 3: استخدام wrangler...${NC}"
        export CLOUDFLARE_API_TOKEN="$API_TOKEN"
        
        if wrangler deploy --name cursor-backend --compatibility-date 2024-01-01 --script simple-backend.js; then
            echo -e "${GREEN}✅ تم رفع الباكيند بنجاح (طريقة 3)${NC}"
        else
